...
$currentUserName = $user->getUserData('display_name');
...
if (($user->perm->checkRight($currentUserId, 'edit_faq') ||
    ...
    if ('editpreview' === $action) {
        ...
        //$mailInfo = $categoryRelation->getMailInfo($faqData['id'], $faqData['lang']);
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="tab-question-answer">
                              ...
                              <form id="faqEditor" action="?action=<?= $queryString ?>" method="post">
                                ...
                                <input type="hidden" name="senderName" id="senderName" value="<?= $currentUserName ?>">
                                <!-- Question -->
                                <div class="form-group">
            ...
            <div class="col-lg-3">
                <div id="accordion" role="tablist">
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-header text-center form-inline" role="tab" id="pmf-heading-handle">
                            <?php if ($selectedRevisionId !== 0): ?>
                                <div class="form-group">
                                    <input type="email" name="mailto" id="mailto" placeholder="E-mail" pattern="[^@]+@[^@]+\.[a-zA-Z]{2,6}"
                                    class="form-control">
                                </div>
                                <div class="form-group text-right" style="margin-left:7px">
                                    <button class="form-control btn btn-lg btn-success" id="mail_btn"
                                        onclick="readyEmail();"
                                    >
                                ...
                                <div class="form-group text-right" style="margin-left:7px">
                                    <button class="form-control btn btn-lg btn-danger" id="delete_btn"
                                        onclick="deleteRecord(<?= $faqData['id'] ?>, '<?= $faqData['lang'] ?>', '<?= $user->getCsrfTokenFromSession() ?>');"
                                    >
  ...
  <div class="modal fade" id="attachmentModal" tabindex="-1" role="dialog" aria-labelledby="attachmentModalLabel"
    ...
    <script>
        ...
        document.getElementById("mail_btn").addEventListener("click", function(event){
        event.preventDefault()
        });

        document.getElementById("mailto").addEventListener("keydown", function(event){
            if (event.keyCode === 13) {
                // Cancel the default action, if needed
                event.preventDefault();
                // Trigger the button element with a click
                document.getElementById("mail_btn").click();
            }
        });
